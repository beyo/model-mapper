...
const DEFAULT_EVENT_PATTERN = 'mapper.{mapper}.{event}';
...
function * defineMapper(mapperName, options) {
  ...
  prototype = yield _preparePrototype(mapperName, options);
...
function _methodEventWrapper(mapperName, eventsOptions) {
  ...
  return function (methodName, method) {
    ...
    if (eventsOptions.mapping[methodName]) {
      action = eventsOptions.mapping[methodName];
      ...
      return function eventWrapper() {
        ...
        (eventsOptions.emitter || events).emit(eventName, action, model);
