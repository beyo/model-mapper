...
function registerAdapter(name, factory) {
  if (!name) {
    throw new ConnectionException('Adapter name must not be empty');
  } else if (typeof name !== 'string') {
    throw new ConnectionException('Adapter name must be a string');
  } else if (!(factory instanceof Function)) {
    throw new ConnectionException('Adapter factory must be a function');
  }
...
function defineConnection(name, connectionString) {
  ...
  if (!name) {
    throw new ConnectionException('Connection name must not be empty');
  } else if (typeof name !== 'string') {
    throw new ConnectionException('Connection name must be a string');
  } else if (!connectionString) {
    throw new ConnectionException('Connection string must not be empty');
  } else if (typeof connectionString !== 'string') {
    throw new ConnectionException('Connection string must be a string');
  }
...
function * getConnection(name) {
  ...
  if (!connection) {
    throw new ConnectionException('Unknown connection name : ' + String(name));
  }

  factory = adapterFactories[connection.adapter];

  if (!factory) {
    throw new ConnectionException('Unknown adapter : ' + String(connection.adapter));
  }
